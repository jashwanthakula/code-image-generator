...
def index():
    ...
    image_data = None
    image_filename = None
    cache_id = session.get('cache_id')  # Get existing cache_id if any

    # Clear session and cache on GET request to ensure a fresh page
    if request.method == "GET":
        session.pop('cache_id', None)
        if cache_id and cache_id in SCREENSHOT_CACHE:
            SCREENSHOT_CACHE.pop(cache_id, None)  # Remove cached data on refresh
    ...
    if request.method == "POST":
        ...
        session.pop('cache_id', None)
        if cache_id and cache_id in SCREENSHOT_CACHE:
            SCREENSHOT_CACHE.pop(cache_id, None)  # Clean up old cache before new generation
